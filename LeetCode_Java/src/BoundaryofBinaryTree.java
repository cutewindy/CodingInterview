 ...
 * root. Boundary includes left boundary, leaves, and right boundary in order without duplicate nodes.
...
public class BoundaryofBinaryTree {
	...
	/**
	 * Pass three times to get left bound, leaf and right bound
	 * @param TreeNode root
	 * @return List<Integer>
	 * Time: O(3n)
	 * Space: O(1)
	...
	private void leaf(TreeNode root, List<Integer> res) {
		if (root == null) return;
	...
	public static void main(String[] args) {
		...
		BoundaryofBinaryTree result = new BoundaryofBinaryTree();
		TreeNode root = TreeNode.generateCBT(new int[] {1, 2, 3, 4, 5, 6, 7, 8, 9});
		TreeNode.printCBT(root);
		System.out.println(result.boundaryofBinaryTree(root));
	}
