...
public class StickerstoSpellWord {
	...
	/**
	 * DP
	 * @param String[] stickers, String target
	 * @return int
	 * Time: O()
	 * Space: O()
	 */
	public int stickerstoSpellWord(String[] stickers, String target) {
        int n = target.length();
        int state = 1 << n;
        int[] dp = new int[state];
		Arrays.fill(dp, Integer.MAX_VALUE);
        dp[0] = 0;
        for (int currState = 0; currState < state; currState++) {
        	...
        	for (String sticker: stickers) {
        		...
        		dp[nextState] = Math.min(dp[currState] + 1, dp[nextState]);
        	}
        }

        return dp[state - 1] == Integer.MAX_VALUE ? -1 : dp[state - 1];
    }

	public static void main(String[] args) {
		// TODO Auto-generated method stub
		StickerstoSpellWord result = new StickerstoSpellWord();
		System.out.println(result.stickerstoSpellWord(new String[] {"with", "example", "science"}, "thehat"));
		System.out.println(result.stickerstoSpellWord(new String[] {"ab", "bc", "ac"}, "abc"));
	}
