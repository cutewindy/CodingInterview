...
public class FrogJump {
	...
	/**
	 * DP: 
	 * dp[i]: the set of units that can jump to ith stone.
	 * On each new stone, we look if any previous stones can reach to it. If so, we update that new 
	 * stone's set of units by adding steps k,
	 * @param int[] stones
	 * @return boolean
	 * Time: O(n^2)
	 * Space: O(n^2)
	...
	public boolean frogJump(int[] stones) {
		...
		for (int end = 1; end < n; end++) {
			...
			for (int start = 0; start < end; start++) {
				int k = stones[end] - stones[start];
				if (dp[start].contains(k - 1) || dp[start].contains(k) || dp[start].contains(k + 1)) {
					dp[end].add(k);
				}
			}
		}
	...
	public static void main(String[] args) {
		...
		FrogJump result = new FrogJump();
		System.out.println(result.frogJump(new int[] {0, 1, 3, 5, 6, 8, 12, 17}));
	}
