...
public class ShortestWordDistanceIII {
	...
	public int shortestWordDistanceIII(String[] words, String word1, String word2) {
		...
		for (int i = 0; i < words.length; i++) {
			...
			else if (words[i].equals(word1)) index1 = i;
			else if (words[i].equals(word2)) index2 = i;
			if (index1 != -1 && index2 != -1) result = Math.min(Math.abs(index1 - index2), result);
		}
		return result;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(result.shortestWordDistanceIII(
				new String[] {"practice", "makes", "perfect", "coding", "makes", "makes"}, "makes", "makes"));
	}
