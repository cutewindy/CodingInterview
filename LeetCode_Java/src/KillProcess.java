...
public class KillProcess {
	...
	/**
	 * Hashmap + DFS
	 * Use hashmap, which stores the data in the form {parent: [list of all direct children]}.
	 * @param List<Integer> pid, List<Integer> ppid, int kill
	 * @return List<Integer>
	 * Time: O(n)
	 * Space: O(n)
	 * Stack space: O(log(n))
	...
	public List<Integer> killProcess(List<Integer> pid, List<Integer> ppid, int kill) {
		...
		killing(map, kill, res);
		return res;
	}
	
	
	private void killing(Map<Integer, List<Integer>> map, int kill, List<Integer> res) {
		res.add(kill);
		if (!map.containsKey(kill)) return;
		for (Integer k: map.get(kill)) {
			killing(map, k, res);
		}
	}
	...
	public static void main(String[] args) {
		...
		KillProcess result = new KillProcess();
		System.out.println(result.killProcess(
				new ArrayList<>(Arrays.asList(1, 3, 10, 5)), new ArrayList<>(Arrays.asList(3, 0, 5, 3)), 5));
	}
